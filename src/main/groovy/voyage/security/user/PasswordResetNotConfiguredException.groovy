...
/**
 * For use within the service layer to inform the caller that the password reset configuration has not been completed for
 * the client attempting to call into the API.
...
class PasswordResetNotConfiguredException extends AppException {
    ...
    PasswordResetNotConfiguredException(String message) {
        super(HTTP_STATUS, message)
    }

    @Override
    String getErrorCode() {
        return ErrorUtils.getErrorCode(httpStatus.value(), 'password_reset_not_configured')
    }
}
